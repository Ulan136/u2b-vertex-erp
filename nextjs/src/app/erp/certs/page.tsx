...
type Cert = {
  ...
  result?: string | null; operStatus?: string | null; payStatus?: string | null; invoiceType?: string | null; sentStatus?: string | null; note?: string | null; unfitReason?: string | null; createdByName?: string | null; createdAt?: string | null; amount?: string | number | null;
...
const PAY = ['В ожидании', 'Оплачено', 'Есть остаток', 'Бесплатно'];
...
const EMPTY = { id: '', fio: '', address: '', phone: '', client: '', meterType: '', serialNo: '', yearMade: '', waterType: 'х/в', checkDate: '', nextCheckDate: '', stampNo: '', sealType: 'СЛ', readings: '', result: 'Годен', operStatus: 'В работе', payStatus: 'В ожидании', invoiceType: '', sentStatus: 'Не отправлено', note: '', unfitReason: '', amount: '', accuracyClass: '', ownerKind: 'физлицо', ownerTaxId: '', addressKz: '', verifier: '' };
...
const VOICE_IZV: Array<[keyof typeof EMPTY, string]> = [['fio', 'ФИО абонента'], ['address', 'Адрес'], ['serialNo', 'Заводской номер'], ['unfitReason', 'Причина непригодности'], ['yearMade', 'Год выпуска'], ['phone', 'Телефон'], ['client', 'Клиент'], ['note', 'Примечание']];
...
function CertsInner() {
  ...
  // ── Выплата комиссии клиенту (только ВДК) — блок 2 общей модалки оплаты ──
  const [commFrom, setCommFrom] = React.useState('');
  const [commTo, setCommTo] = React.useState('');
  const [commPer, setCommPer] = React.useState('200');           // комиссия за сертификат
  ...
  const hasComm = source === 'ВДК' && isCert;   // блок «Выплата комиссий» — только ВДК
  ...
  const list = all.filter(c => {
    ...
    // Порядок реестра: по ДАТЕ ПОВЕРКИ (возрастанию) — исправил дату, серт встаёт
    // на своё место (напр. 08.09 идёт выше 09.09), а не остаётся по дате создания.
    // При равной дате — по времени создания (нумерация стабильна). Без даты — вниз.
    .sort((a, b) =>
      (iso(a.checkDate) || '9999-99-99').localeCompare(iso(b.checkDate) || '9999-99-99')
  ...
  function openPay() {
    if (!hasComm && !fClient) { toast('Сначала выберите клиента в фильтре'); return; }
    // Блок 1 — приём оплаты. Счёт и сумма НЕ подставляются автоматически (пустые):
    // кассир выбирает счёт и вписывает принятое явно (иначе можно случайно провести
    // всю сумму на счёт по умолчанию, не заметив).
    setPcErr(''); setPcPrice(''); setPcFrom(''); setPcTo(''); setPcQty(String(pcCount)); setPcRows([{ accountId: '', amount: '' }]);
  ...
  async function markCommissionPaid() {
    if (commCount === 0) { setCommErr('Нет сертификатов для отметки за период'); return; }
    if (!confirm(`Отметить комиссию ВЫПЛАЧЕННОЙ (без движения денег) для ${commCount} серт.?\nИспользуйте для прошлых периодов, где комиссия уже отдана.`)) return;
    setCommSaving(true); setCommErr('');
    try {
  ...
  async function payClientSubmit() {
    ...
    // Подтверждение: сертификаты оплачиваются, но комиссия остаётся нашим долгом.
    if (canOffset && !confirm(`Оплата сертификатов: ${fmtNum(paidSum)} ₸.\nКомиссия ${fmtNum(offsetComm)} ₸ ОСТАЁТСЯ нашим долгом (Блок 2) — взаиморасчёт НЕ проводится.\n\nПровести?`)) return;
    ...
    <div>
      ...
      {isDirect && fClient && (
        <div className="cert-payclient-bar">
          ...
          {pcCount > 0 && <Button variant="outline" onClick={openPay} style={{ marginLeft: 'auto', fontSize: 12 }}>🧾 Приём оплаты за {pcCount} серт.</Button>}
        </div>
      )}
              ...
              <tbody>
                {list.map((c, i) => (
                  <tr key={c.id} data-focus-id={c.id} className={isTTE(c) ? 'cert-hot' : ''} style={!trash && paidNoIncome(c) ? { background: '#fef08a' } : undefined} title={!trash && paidNoIncome(c) ? '⚠ Оплачено, но доход не проведён — укажите цену и счёт (после исправления подсветка исчезнет)' : undefined}>
                    ...
                    <td style={{ fontSize: 11, maxWidth: 240, whiteSpace: 'normal' }} title={c.unfitReason || undefined}>{c.unfitReason || '—'}</td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} width={680}
        ...
        {(!isCert || form.result === 'Не годен') && (
          <Field label="Причина непригодности (в е-КТРМ)"><div className="cert-vf"><Input value={form.unfitReason} onChange={e => setForm({ ...form, unfitReason: e.target.value })} placeholder="напр. погрешность выше нормы, механическое повреждение…" /><Mic k="unfitReason" h="Причина непригодности" /><Copy v={form.unfitReason} h="Причина непригодности" /></div></Field>
        )}
        <div className="erp-form-row">
      ...
      {/* Модалка «Оплата сертификата / выплата комиссий» — 2 блока (комиссия только ВДК) */}
      <Modal open={pcOpen} onClose={() => setPcOpen(false)} width={580}
        title={<span>🧾 Оплата сертификата{hasComm ? ' / выплата комиссий' : ''}</span>}
        footer={<Button variant="outline" onClick={() => setPcOpen(false)}>Закрыть</Button>}>

        {/* ── Блок 1 · Приём оплаты сертификата ── */}
        <div className="cert-sec-lbl" style={{ fontSize: 14, fontWeight: 700, marginTop: 0 }}>1 · 🧾 Приём оплаты сертификата{fClient ? ` — ${fClient}` : ''}</div>
        {!fClient ? (
          <div className="erp-muted" style={{ fontSize: 13, margin: '4px 0 8px' }}>Выберите клиента в фильтре «Клиент» — тогда можно принять оплату по его сертификатам.</div>
        ) : (<>
          ...
          <div className="sale-pay" style={{ marginTop: 0 }}>
            ...
            <Button variant="outline" onClick={() => setPcRows(rs => [...rs, { accountId: '', amount: '' }])} style={{ fontSize: 12 }}>+ ещё счёт</Button>
            <div className="sale-pay-state">
            ...
            {/* Сообщения об остатке — только когда введён кэш; считаем ПОСЛЕ взаиморасчёта (pcGap) */}
            {canOffset && pcPayTotal <= 0 && <div className="erp-muted" style={{ fontSize: 12, marginTop: 4 }}>Впишите принятую сумму — взаиморасчёт покроет комиссию {fmtNum(offsetComm)} ₸.</div>}
            {canOffset && pcPayTotal > 0 && Math.abs(pcGap) < 0.5 && <div style={{ fontSize: 12, color: '#16a34a', marginTop: 4 }}>✓ Полностью покрыто: принято {fmtNum(pcPayTotal)} + взаиморасчёт {fmtNum(offsetComm)} = итог {fmtNum(pcIncomeTotal)} ₸.</div>}
            {canOffset && pcPayTotal > 0 && pcGap > 0.5 && <div style={{ fontSize: 12, color: '#b45309', marginTop: 4 }}>Недоплата <b>{fmtNum(pcGap)} ₸</b> и после взаиморасчёта — последний покрытый сертификат станет «Есть остаток», остальные — «В ожидании».</div>}
            {canOffset && pcPayTotal > 0 && pcGap < -0.01 && <div style={{ fontSize: 12, color: '#dc2626', marginTop: 4 }}>Принято + взаиморасчёт больше итога на <b>{fmtNum(-pcGap)} ₸</b> — уменьшите сумму.</div>}
            {!canOffset && pcLeftover > 0.5 && pcPayTotal > 0 && <div style={{ fontSize: 12, color: '#b45309', marginTop: 4 }}>Недоплата <b>{fmtNum(pcLeftover)} ₸</b> — последний покрытый сертификат станет «Есть остаток», остальные — «В ожидании».</div>}
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <Button variant={canOffset ? 'outline' : undefined} onClick={payClientSubmit} disabled={pcSaving || pcOver || pcPriceNum <= 0 || pcQtyNum <= 0} title={pcQtyNum <= 0 ? 'Нет сертификатов за период' : pcOver ? 'Принято больше итога' : canOffset ? `Сертификаты оплачены; комиссия ${fmtNum(offsetComm)} ₸ остаётся нашим долгом (Блок 2)` : undefined}>{pcSaving ? 'Проведение…' : pcQtyNum <= 0 ? 'Нет за период' : pcOver ? '⚠ Больше итога' : (canOffset ? '💾 Оплата без зачёта (комиссия — наш долг)' : '💾 Принять оплату')}</Button>
          </div>
        </>)}
        ...
        {hasComm && (<>
          ...
          <div className="erp-form-row">
            ...
            <Field label="По дату"><Input type="date" value={commTo} min={commFrom || undefined} onChange={e => setCommTo(e.target.value)} /></Field>
          </div>
          <div className="erp-form-row">
