...
type Org = { companyName?: string | null; bin?: string | null; address?: string | null; directorName?: string | null; banks?: { key: string; name: string; iik?: string; bik?: string; kbe?: string }[] | null; logoB64?: string | null; stampB64?: string | null; signB64?: string | null };
...
export default function DocumentsPage() {
  ...
  async function print(id: string) {
    ...
    const o = (org || {}) as Org;
    const type = String(d.type || 'invoice');
    const M = (n: unknown) => (Number(n) || 0).toLocaleString('ru-RU');
    const its = (d.items as { name?: string; sku?: string; qty?: number; unit?: string; price?: number; sum?: number }[]) || [];
    const logo = o.logoB64 ? `<img src="${o.logoB64}" style="height:52px">` : '';
    const sign = d.withSign && o.signB64 ? `<img src="${o.signB64}" style="position:absolute;left:132px;top:-30px;height:58px">` : '';
    const stamp = d.withStamp && o.stampB64 ? `<img src="${o.stampB64}" style="position:absolute;left:214px;top:-72px;height:120px;opacity:.92">` : '';
    const dir = o.directorName || 'Молдабаев М.А.';
    let html: string;
    if (type === 'invoice') {
      const banks = o.banks || [];
      const bank = banks.find(b => b.key === (d.bank || 'kaspi')) || banks[0] || { name: '', iik: '', bik: '', kbe: '' };
      const rows = its.map((it, i) => { const sum = it.sum ?? (Number(it.qty) * Number(it.price)); return `<tr><td class="c">${i + 1}</td><td class="c">${it.sku || ''}</td><td class="l">${it.name || ''}</td><td class="c">${it.qty ?? ''}</td><td class="c">${it.unit || 'шт'}</td><td class="r">${M(it.price)}</td><td class="r">${M(sum)}</td></tr>`; }).join('');
      html = `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Счет на оплату № ${d.number}</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font-family:'Times New Roman',serif;font-size:11pt;color:#000;margin:0}.hdr{display:flex;gap:12px;align-items:flex-start;margin-bottom:8px}.warn{flex:1;font-size:7.5pt;font-style:italic;line-height:1.25}table{border-collapse:collapse;width:100%}.bank td{border:1px solid #000;padding:3px 6px;font-size:9.5pt;vertical-align:top}.title{text-align:center;font-weight:bold;font-size:15pt;margin:12px 0 10px}.p{margin:3px 0}.p b{display:inline-block;min-width:82px;vertical-align:top}.items td,.items th{border:1px solid #000;padding:4px 5px;font-size:9.5pt}.items th{font-weight:bold;text-align:center}.c{text-align:center}.l{text-align:left}.r{text-align:right;white-space:nowrap}.tot{text-align:right;font-weight:bold;margin:4px 0}.vsego{font-weight:bold;margin:8px 0;border-top:2px solid #000;border-bottom:2px solid #000;padding:5px 0;font-size:12pt}.sig{position:relative;margin-top:38px;font-weight:bold}</style></head><body><div class="hdr"><div>${logo}</div><div class="warn">Внимание! Оплата данного счета означает согласие с условиями поставки товара. Товар отпускается по факту прихода денег на р/с Поставщика, самовывозом, при наличии доверенности и документов, удостоверяющих личность.</div></div><table class="bank"><tr><td style="width:46%"><b>Бенефициар:</b><br>${o.companyName || ''}, БИН ${o.bin || ''}</td><td style="width:34%">ИИК<br>${bank.iik || ''}</td><td>Кбе<br>${bank.kbe || ''}</td></tr><tr><td><b>Банк бенефициара:</b><br>${bank.name || ''}</td><td>БИК<br>${bank.bik || ''}</td><td>Код назначения платежа<br>710</td></tr></table><div class="title">Счет на оплату № ${d.number} от ${dmy(d.docDate as string)} г.</div><div class="p"><b>Поставщик:</b> БИН / ИИН: ${o.bin || ''}, ${o.companyName || ''}, ${o.address || ''}</div><div class="p"><b>Покупатель:</b> ${d.buyerBin ? 'БИН / ИИН: ' + d.buyerBin + ', ' : ''}${d.buyerName || ''}${d.buyerAddress ? ', ' + d.buyerAddress : ''}</div><div class="p"><b>Договор:</b> б/д</div><table class="items" style="margin-top:8px"><thead><tr><th style="width:5%">№</th><th style="width:9%">Код</th><th style="width:43%">Наименование</th><th style="width:9%">Кол-во</th><th style="width:7%">Ед.</th><th style="width:12%">Цена</th><th style="width:15%">Сумма</th></tr></thead><tbody>${rows}</tbody></table><div class="tot">Итого: ${M(d.total)}</div><div class="tot" style="font-weight:normal">Без НДС</div><div class="vsego">Всего к оплате:&nbsp;&nbsp;${d.amountWords || ''}</div><div class="sig">Исполнитель&nbsp;&nbsp;__________________${sign}${stamp}&nbsp;&nbsp;/ ${dir} /</div><script>window.onload=()=>window.print()<\/script></body></html>`;
    } else {
      const rows = its.map((it, i) => `<tr><td>${i + 1}</td><td style="text-align:left">${it.name || ''}</td><td>${it.qty ?? ''}</td><td>${it.unit || 'шт'}</td><td style="text-align:right">${M(it.price)}</td><td style="text-align:right">${M(it.sum ?? (Number(it.qty) * Number(it.price)))}</td></tr>`).join('');
      html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${d.docNo || 'Документ'}</title><style>@page{size:A4;margin:14mm}body{font-family:'Times New Roman',serif;font-size:12px}h2{text-align:center}table{width:100%;border-collapse:collapse}td,th{border:1px solid #000;padding:4px;text-align:center}</style></head><body><h2>${d.docNo || ''} от ${dmy(d.docDate as string)} г.</h2><div><b>Поставщик:</b> ${o.companyName || 'ТОО «VERTEX SERVICE»'}, БИН ${o.bin || ''}</div><div style="margin-bottom:8px"><b>Покупатель:</b> ${d.buyerName || ''}${d.buyerBin ? ', БИН ' + d.buyerBin : ''}</div><table><thead><tr><th>№</th><th>Наименование</th><th>Кол-во</th><th>Ед.</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>${rows}</tbody></table><div style="margin-top:6px"><b>Итого:</b> ${M(d.total)} · <b>${d.amountWords || ''}</b></div><div style="position:relative;margin-top:36px"><b>Исполнитель</b> _____________ ${sign}${stamp} / ${dir} /</div><script>window.onload=()=>window.print()<\/script></body></html>`;
    }
